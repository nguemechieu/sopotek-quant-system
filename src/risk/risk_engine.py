...
class RiskEngine:
    ...
    def __init__(
        ...
        *,
        ...
        self.limits = limits or RiskLimits(
            ...
            max_drawdown_limit_pct=float(max_drawdown_limit or daily_drawdown_limit or self.config.max_portfolio_drawdown),
    ...
    # Backward-compatible aliases used by legacy runtime components and tests.
    ...
    def review_signal(self, signal: Signal | TraderDecision | Mapping[str, Any]) -> TradeReview:
        normalized = self._coerce_signal(signal)
        ...
        if "loss_streak_pause" in performance_constraints:
            return self._reject(normalized, "Loss-streak pause triggered by centralized risk engine", risk_constraints=performance_constraints)
    ...
    def _roll_session(self, timestamp, equity: float) -> None:
    ...
    def _coerce_signal(self, value: Signal | TraderDecision | Mapping[str, Any]) -> Signal:
    ...
    def _symbol_exposure_cap(self, signal: Signal) -> float:
        profile_cap = {"low": 0.12, "medium": 0.18, "high": 0.25}.get(self._risk_level(signal), self.limits.per_symbol_exposure_cap_pct)
    ...
    def _gross_exposure_cap(self, signal: Signal) -> float:
        profile_cap = {"low": 0.55, "medium": 0.80, "high": 1.00}.get(self._risk_level(signal), self.limits.max_portfolio_exposure_pct)
