...
def normalize_order_history_entry(order):
    ...
    symbol = str(order.get("symbol") or order.get("instrument") or "").strip()
    order_id = str(order.get("id") or order.get("order_id") or order.get("clientOrderId") or "").strip()
    if not symbol and not order_id:
        return None

    amount = abs(float(order.get("amount") or order.get("qty") or order.get("quantity") or order.get("size") or 0.0) or 0.0)
    filled = abs(float(order.get("filled") or order.get("filled_qty") or order.get("executedQty") or 0.0) or 0.0)
    remaining = order.get("remaining")
    remaining_value = _coerce_float(remaining, default=max(0.0, amount - filled))
    price = _coerce_float(order.get("price"), default=_coerce_float(order.get("average"), default=None))

    return {
        "timestamp": str(order.get("timestamp") or order.get("datetime") or order.get("time") or "").strip(),
        "symbol": symbol,
        "side": str(order.get("side") or "").strip().lower(),
        "type": str(order.get("type") or order.get("order_type") or "").strip().lower(),
        "price": price,
        "filled": float(filled),
